...
export class LUNCDashWalletConnectV1 extends ConnectedWallet {
  ...
  private async sendRequest<T>(method: string, params: unknown): Promise<T> {
    ...
    if (isMobile()) {
      ...
      window.location.href = `luncdash://walletconnect_confirm/?action=walletconnect_confirm&payload=${payload}`;
    }
    try {
